...
function M.parse(arg)
   ...
   cmd:option('-LR_D',             2.5e-4, 'Learning rate of discriminator')
   ...
   cmd:option('-schedule',     '140 160 180', 'schedule to decay learning rate')
   cmd:option('-gamma',        0.4,        'LR is multiplied by gamma on schedule.')
   ---------- Model options ----------------------------------
   ...
   cmd:option('-netType',      'hg-atten',   'Options: hg-atten | hg')
   cmd:option('-shortcutType', '',         'Options: A | B | C')
   cmd:option('-retrain',      'none',     'Path to model to retrain with')
   cmd:option('-optimState',   'none',     'Path to an optimState to reload from')
   cmd:option('-nStack',       7,          'Number of stacks in the provided hourglass model (for hg-generic)')
   ...
   cmd:option('-lambda_G',       0.1, 'Weight of G1 and G2')
   cmd:option('-lambda_k',        0.001, 'Weight to clip k_t')
   cmd:option('-gamma_gd',             0.5, 'Balance weight of real and fake')
